...
export function WalletDrawer({ item }: { item: Wallet }) {
  ...
  return (
    <Drawer direction="right">
      ...
      <DrawerContent className="sm:max-w-md max-h-screen overflow-y-auto overflow-x-hidden">
        ...
        {extraMetrics && (
          <div className="px-4 py-2 space-y-2 text-sm border-t mt-4">
            ...
            />
            <LabelWithValue label="UTXO Count" value={extraMetrics.utxoCount} />
            <LabelWithValue
              label="Total Funded"
            ...
            <ul className="space-y-2 max-h-72 overflow-y-auto pr-2">
              {utxoList.map((utxo, idx) => (
                ...
                >
                  ...
                  <div className="text-xs">
                    <strong>Value:</strong> {(utxo.value / 1e8).toFixed(8)} BTC
                  </div>
                  <div className="text-xs">
